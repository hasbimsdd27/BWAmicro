...
class ReviewController extends Controller
{
    public function create(Request $request){
        ...
        if($isExistReview){
            return response()->json(['status'=>'error', 'message'=>'review already exist'],409);
        }

        $isTakenCourse = MyCourse::where('course_id','=',$courseId)->where('user_id','=',$userId)->exists();
        if(!$isTakenCourse){
            return response()->json(['status'=>'error', 'message'=>'user is not take this course'],403);
        }
